/****** MAPAPI.H *************** Belenkov O.V.    ***** 06/08/08 ***
******* MAPAPI.H *************** Tarakanova I.R.  ***** 13/11/07 ***
******* MAPAPI.H *************** Panteleeva N.A.  ***** 24/03/08 ***
******* MAPAPI.H *************** Shishkova L.A.   ***** 20/04/06 ***
******* MAPAPI.H *************** Kruzhkova E.A.   ***** 03/03/08 ***
******* MAPAPI.H *************** Shabakov D.A.    ***** 31/01/06 ***
******* MAPAPI.H *************** Dishlenko S.G.   ***** 03/08/05 ***
******* MAPAPI.H *************** Kruzhkov A.E.    ***** 31/10/06 ***
*                                                                  *
...
#define MAPACCESSVERSION  20070104

// Версия интерфейса MAPAPI
#define MAPAPIVERSION  0x091503
...
_MAPIMP long int _MAPAPI mapPaintToImage(HMAP hmap, RGBQUAD * palette,
 ...
 // При ошибке в параметрах возвращает ноль
  ...
  // Вывести изображение карты и объекта в XImage (массив)         // 03/03/07
  // Данная функция реализована для XWindow !
  // Описание структуры XIMAGEDESC в maptype.h
  // x,y - координаты левого верхнего угла внутри
  // битовой области XImage для размещения изображения
...
_MAPIMP long int _MAPAPI mapViewMapObject(HMAP hmap,HWND hwnd,DOUBLEPOINT * point,
                                          PAINTPARM * image,
                                          HOBJ info,
#ifdef __cplusplus   // 09/07/01
                                          long int place = PP_MAP);
#else
                                          long int place);
#endif

#ifdef __cplusplus   // 09/10/02
 // Отобразить произвольный объект в пределах фрагмента окна (карты)
 // в условных знаках пользователя
 // hmap   - идентификатор открытых данных
 // image - описание вида объекта (см. MAPGDI.H), если объект
...
_MAPIMP void _MAPAPI mapCloseImage(HIMAGE himage);
...
_MAPIMP HDC _MAPAPI mapGetImageDC(HIMAGE himage);
  ...
  // Запросить контекст буфера окна для отображения объектов   // 12/01/08
  // himage - идентификатор буфера окна
  // При ошибке возвращает ноль

_MAPIMP HDC _MAPAPI mapGetImageObjectDC(HIMAGE himage);
...
_MAPIMP long int _MAPAPI mapViewImage(HIMAGE himage, HDC hdc, RECT * rect);
...
_MAPIMP long int _MAPAPI mapViewImageEx(HIMAGE himage, HWND hwnd, RECT * rect);
...
_MAPIMP long int _MAPAPI mapChangeImageSizeEx(HIMAGE himage, long int erase,
                                              long int width, long int height);
...
_MAPIMP long int _MAPAPI mapChangeImageSize(HIMAGE himage);
...
_MAPIMP long int _MAPAPI mapDrawImageMap(HIMAGE himage, HMAP hMap, RECT * rect,
                                         POINT * position);
...
_MAPIMP long int _MAPAPI mapDrawImageOffsetUserObject(HIMAGE himage, HMAP hMap,
  ...
  // himage   - идентификатор образа экрана,
...
_MAPIMP long int _MAPAPI mapDrawImageGraphics(HIMAGE himage, HMAP hMap,
                                              DRAWPOINT * points, long int count,
                                              long int type, const char * parm);

  // Отобразить текстовую строку (Arial)        // 16/07/08
...
_MAPIMP void _MAPAPI mapClearImageObjects(HIMAGE himage, RECT * rect);
...
_MAPIMP long int _MAPAPI mapGetObjectScale(HOBJ info);   // 27/12/05
...
_MAPIMP long int _MAPAPI mapSemanticCode(HOBJ info,long int number);
 ...
 // Запросить последовательный номер кода семантической характеристики объекта
 // (c 1)                                                          // 20/07/06
 // info    - идентификатор объекта карты в памяти
...
_MAPIMP long int _MAPAPI mapSemanticClassificatorCode(HOBJ info,
 ...
 // Пример:входная строка - "дом № %#45[нет] сост. %#3[не заполнено]"     *
 //     результат по значениям семантики для конкретного объекта          *
 //     "дом № 5 сост. не заполнено" или "дом № 7-a сост. ЖИЛОЙ"          *
 //      или  "дом № нет сост. не заполнено"                              *
 // ***********************************************************************
...
_MAPIMP double _MAPAPI mapHPlane(HOBJ info,
 ...
 // Запросить координаты точки                   // 05/08/08
 // number - номер точки (начинается с 1)
 // subject - номер подобъекта (если = 0, обрабатывается объект)
 // При ошибке возвращает ноль
...
#ifdef MULTIPLATFORM                            // 12/07/07
  //  Запросить путь к данным платформы Sparc   // 20/04/07
...
_MAPIMP void _MAPAPI mapGetTurnPathEx(char * path, long int size);
  ...
  // Сохранить данные при работе на  Sparc в Windows-формате (до закрытия данных)   // 10/07/07
...
_MAPIMP long int _MAPAPI mapGetMapScreenDepth();   // 21/03/03
  ...
  *********************************************************/
  ...
  // Установить предельные размеры буфера изображения           // 03/07/07
...
_MAPIMP long int _MAPAPI mapGetScreenWidth();
  ...
  // экрана в миллиметрах (100 - 2000). При установке возвращает
  // старое значение
...
_MAPIMP double _MAPAPI mapScreenPixel2Meter(long int pixel);
 ...
 /*********************************************************
 *                                                        *
 *           ФУНКЦИИ ОБРАБОТКИ INI ФАЙЛА                  *
 *                                                        *
 *********************************************************/

typedef long int HINI;

  // Открыть секцию в INI-файле                          // 21/07/08
  // section  - имя секции (без квадратных скобок)
  // filename - полный путь к INI-файлу
  // После завершения работы с INI-файлом необходимо закрыть секцию
  // При ошибке возвращает ноль, иначе - идентификатор открытой секции

_MAPIMP HINI _MAPAPI mapOpenProfile(const TCHAR * section, const TCHAR * filename);

  // Закрыть секцию в INI-файле
  // hini  - идентификатор секции

_MAPIMP void _MAPAPI mapCloseProfile(HINI hini);

  // Записать обновленные данные в INI-файл
  // При закрытии INI-файла данные сохранятся автоматически
  // hini  - идентификатор секции

_MAPIMP void _MAPAPI mapCommitProfile(HINI hini);

  // Запросить значение параметра в виде строки по ключу
  // hini  - идентификатор секции
  // key   - значение ключа параметра
  // value - адрес строки для размещения результата
  // size  - размер строки для размещения результата
  // defvalue - значение строки, если параметр не будет найден по ключу
  // При ошибке возвращает ноль

_MAPIMP long int _MAPAPI mapGetProfileString(HINI hini, const TCHAR * key,
                                             TCHAR * value, long int size,
                                             const TCHAR * defvalue);

  // Запросить значение параметра в виде числа по ключу
  // hini  - идентификатор секции
  // key   - значение ключа параметра
  // defvalue - возвращаемой значение, если параметр не будет найден по ключу
  // Возвращает указатель на значение параметра

_MAPIMP long int _MAPAPI mapGetProfileInt(HINI hini, const TCHAR * key, long int defvalue);

  // Записать значение параметра в виде строки по ключу
  // hini  - идентификатор секции
  // key   - значение ключа параметра
  // value - значение параметра
  // При ошибке возвращает ноль

_MAPIMP long int _MAPAPI mapWriteProfileString(HINI hini, const TCHAR * key, const TCHAR * value);

  // Записать значение параметра в виде целого числа по ключу
  // hini  - идентификатор секции
  // key   - значение ключа параметра
  // value - значение параметра
  // При ошибке возвращает ноль

_MAPIMP long int _MAPAPI mapWriteProfileInt(HINI hini, const TCHAR * key, long int value);
